...
function start () {
  ...
  sockets.on("connection", socket => {
    ...
    Object.entries(socketMethodStore).forEach(([name, getSocketFunc]) => {
      socket[name] = getSocketFunc(socket);
    });
...
export function socketMethod (name, func) {
  socketMethodStore[name] = func;
}
...
socketMethod("set", socket => (name, doc) => {
  console.log("[hidden] set", name, doc);

  return socket.emit(`${name}::set`, doc);
});

socketMethod("sub", socket => (name, id, initialDoc) => {
  console.log("[hidden] sub", name, id);
  socket.join(`${name}::${id}`);

  if (initialDoc) {
    socket.set(name, initialDoc);
  }

  return socket;
});

socketMethod("unsub", socket => (name, id) => {
  console.log("[hidden] unsub", name, id);
...
socketMethod("update", socket => (name, change) => {
  console.log("[hidden] update", name, change);

  return (socket
    .emit(`${name}::update`, change));
});
